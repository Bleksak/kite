...
pub fn git_snapshot_tree(cwd: Option<&std::path::Path>) -> Option<String> {
                    ...
                    if let Some(phase) = from_gate {
                        match (phase, input.is_empty()) {
                            ...
                            (GatePhase::ReviewStep, true) => {
                                let baseline = git_snapshot_tree(None);
                                let _ = event_tx.send(TuiEvent::ReviewBaseline {
                                    session: id,
                                    baseline,
                                });
...
mod test {
    ...
    #[test]
    fn git_snapshot_captures_new_and_modified_files() {
        ...
        let tree1 = super::git_snapshot_tree(Some(&dir)).expect("first snapshot");
        std::fs::write(dir.join("a.txt"), "one\ntwo\n").unwrap();
        std::fs::write(dir.join("b.txt"), "new file\n").unwrap();
        let tree2 = super::git_snapshot_tree(Some(&dir)).expect("second snapshot");

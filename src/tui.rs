    ...
    scrollback
        ...
        .map(|(line, block)| {
            let (text_background, block_background) = match block {
                BlockKind::User => (None, Some(Color::Rgb(100, 100, 100))),
...
mod test {
    ...
    fn display_lines_pad_user_and_thinking_blocks_to_full_width() {
        ...
        assert_eq!(user.spans[0].style.bg, Some(Color::Rgb(100, 100, 100)));
        assert_eq!(user.spans.last().unwrap().style.bg, Some(Color::Rgb(100, 100, 100)));

...
fn format_review_comments(comments: &[ReviewComment]) -> String {
    ...
    for comment in comments {
        let range = crate::components::code_review::review_comment_range_label(comment);
...
pub(crate) fn word_left(input: &str, cursor: usize) -> usize {
...
pub(crate) fn word_right(input: &str, cursor: usize) -> usize {
    let chars: Vec<char> = input.chars().collect();
    let len = chars.len();
    let mut pos = cursor;

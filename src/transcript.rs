...
impl StyleSheet for TuiStyleSheet {
    ...
    fn heading(&self, level: u8) -> Style {
        let style = Style::default().fg(Color::Rgb(0xf0, 0xc6, 0x74)).bold();
    ...
    fn blockquote(&self) -> Style {
        Style::default().fg(Color::Rgb(128, 128, 128)).italic()
    }
}
...
impl TuiRenderer {
                ...
                if line.is_empty() {
                    ...
                    self.push_line(
                        Line::from(Span::styled(
                            strip_vs16(line),
                            Style::default().bold().fg(Color::Rgb(212, 212, 212)),
                        )),
                        BlockKind::User,
                    );
                }
        ...
        match event {
            ...
            AgentEvent::ToolStarted { header, body } => {
                ...
                self.tool_blocks.insert(
                    header.clone(),
                    (self.scrollback.len(), self.scrollback.len()),
                );
    ...
    fn render_answer_markdown(&mut self) {
        ...
        let lines = style_markdown_lines(render_markdown_lines(&self.turn_answer), None, None);
        ...
        .map(|line| {
            ...
            let spans: Vec<Span> = match restore.get(&original) {
                ...
                None => line
                    ...
                    .iter()
                    .map(|span| Span {
                        content: std::borrow::Cow::Owned(strip_vs16(span.content.as_ref())),
